...
$sql = "SELECT * FROM blogs ORDER BY created_at DESC"; // Adjust to your database schema
...
<body>
    <header>
        <a href="homepage.php" class="back-arrow"><i class="fas fa-arrow-left"></i></a> <!-- Back arrow link -->
        ...
        <div class="blog-container">
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    // Define a character limit for the "Read More" feature
                    $character_limit = 100; // Set your desired character limit
                    
                    // Check the length of the content
                    if (strlen($row['content']) > $character_limit) {
                        // If the content is long, display a summary with "Read More"
                        echo '<p>' . htmlspecialchars(substr($row['content'], 0, $character_limit)) . '...</p>';
                        echo '<a href="read_post.php?id=' . htmlspecialchars($row['id']) . '" class="read-more">Read More</a>'; // Link to full post
                    } else {
                        // If the content is short, display it fully without "Read More"
